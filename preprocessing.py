...
def preprocess_frame(frame, 
    ...
    """
    Предобработка кадра для улучшения качества детекции объектов.
    ...
    enable_upscale : bool
        Включить upscaling с помощью ESRGAN
    ...
    upscale_factor : int
        Коэффициент увеличения для ESRGAN (2 или 4)
    upscale_model : str
        Название модели ESRGAN
    ...
    # 0. Upscaling с помощью ESRGAN (применяется первым, если включен)
    if enable_upscale:
        processed = upscale_frame(processed, upscale_factor, upscale_model)
